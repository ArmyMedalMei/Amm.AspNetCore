...
namespace Amm.NetworkMark.Core.Extentions
{
    /// <summary>
    ...
    public static class StringExtensions
    {
        ...
        public static string Substring(this string source, string startString, params string[] endStrings)
        {
            ...
            foreach (var endString in endStrings)
            {
                ...
                if (endIndex < 0 || endIndex < startIndex) continue;
                break;
            }

            if (endIndex < 0 || endIndex < startIndex)
                throw new InvalidOperationException($"在源字符串中无法找到“{endStrings.JoinAsString()}”的子串位置");
        ...
        public static bool IsIdentityCardId(this string value)
        {
            ...
            if (!regex.Match(value).Success) return false;
            array = regex.Split(value);
            if (!DateTime.TryParse(string.Format("{0}-{1}-{2}", array[2], array[3], array[4]), out time)) return false;
            //校验最后一位
        ...
        /// <returns></returns>
        public static string IsNullOrEmpytToValue(this string value, string nullReplaceValue)
        {
            return value.IsNullOrWhiteSpace() ? nullReplaceValue : value;
        }
        ...
        public static string GetUrlQuery(this string url, string key)
        {
            var uri = new Uri(url);
            var query = uri.Query;
            if (query.IsNullOrEmpty()) return string.Empty;
        ...
        public static string UpperToLowerAndSplit(this string str)
        {
            ...
            while (str.Length > 0)
            {
                var c = str.FirstOrDefault(m => char.IsUpper(m));
                if (c == default(char))
                {
                    words.Add(str);
                    break;
                ...
                var upperIndex = str.IndexOf(c);
                if (upperIndex < 0) //admin
                    return str;
                if (upperIndex > 0) //adminAdmin
                {
                    var first = str.Substring(0, upperIndex);
                    words.Add(first);
        ...
        public static string LowerFirstChar(this string str)
        {
            ...
            if (str.Length == 1) return char.ToLower(str[0]).ToString();
            ...
            bool ignoreCase = false)
        {
            ...
            for (var i = 0; i <= m; i++) mn[i, 0] = i;
            for (var j = 1; j <= n; j++) mn[0, j] = j;
            for (var i = 1; i <= m; i++)
            {
                var c = from[i - 1];
                for (var j = 1; j <= n; j++)
